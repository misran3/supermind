/**
 * AWS Lambda handler for Gmail webhook events
 */
...
const logger = new Logger({ serviceName: "gmail-sync-webhook" });
const tracer = new Tracer({ serviceName: "gmail-sync-webhook" });

/**
 * Webhook handler class for processing Composio Gmail events
 */
class WebhookHandler {
  ...
  async processWebhook(eventBody: ComposioWebhookPayload): Promise<WebhookResponse> {
    try {
      ...
      const { entity_id: entityId, trigger_name: triggerName, data: emailData } = eventBody;
      ...
      logger.info("Processing email webhook", { entityId, triggerName });

      // Enhance the email using AI
      const enhancedContent = await this.emailAgent.processEmail(emailData);
      ...
      const response = await this.memoryManager.addMemory({
        ...
        source: MemorySource.EMAIL_SYNC,
        customId: emailData.id || emailData.message_id,
      });

      logger.info("Successfully stored email memory", {
...
export const lambdaHandler = async (
  ...
  logger.info("Received email webhook event", { event });
